...
namespace timw255.Sitefinity.CustomErrorPages
{
    ...
    public static class CustomErrorPagesInstaller
    {
        ...
        {
            Bootstrapper.Initialized += CustomErrorPagesInstaller.OnBootstrapperInitialized;
        }
        #endregion
        ...
        private static void OnBootstrapperInitialized(object sender, ExecutedEventArgs e)
        {
            if (e.CommandName == "RegisterRoutes")
            {
                ...
                CustomErrorPagesInstaller.RegisterModule();
            }
        }

        /// <summary>
        /// Registers the CustomErrorPages module.
        /// </summary>
        private static void RegisterModule()
        {
            var configManager = ConfigManager.GetManager();
            var modulesConfig = configManager.GetSection<SystemConfig>().ApplicationModules;
            if (!modulesConfig.Elements.Any(el => el.GetKey().Equals(CustomErrorPagesModule.ModuleName)))
            {
                modulesConfig.Add(CustomErrorPagesModule.ModuleName, new AppModuleSettings(modulesConfig)
                {
                    Name = CustomErrorPagesModule.ModuleName,
                    Title = CustomErrorPagesModule.ModuleTitle,
                    Description = CustomErrorPagesModule.ModuleDescription,
                    Type = typeof(CustomErrorPagesModule).AssemblyQualifiedName,
