...
namespace timw255.Sitefinity.CustomErrorPages.Data.EntityFramework.Decorators
{
    public class CustomErrorPagesEFDataProviderDecorator : IDataProviderDecorator, IExecutionStateDecorator
    {
        ...
        public void Initialize(string providerName, NameValueCollection config, Type managerType)
        {
            ...
            var provider = this.DataProvider as ICustomErrorPagesEFDataProvider;

            if (provider != null)
            {
                CustomErrorPagesEFDataConnection connection = CustomErrorPagesEFDataConnection.InitializeConnection(this.connectionName, provider);

                provider.ProviderContext = new CustomErrorPagesEFDataProviderContext()
                {
        ...
        {
            var dataProvider = (ICustomErrorPagesEFDataProvider)this.DataProvider;
        ...
        /// <returns></returns>
        private CustomErrorPagesEFDbContext GetContext(ICustomErrorPagesEFDataProvider provider)
        {
            if (provider.ProviderContext == null)
                throw new Exception(String.Format("Provider {0} is not initialized.", provider.Name));

            return CustomErrorPagesEFDataConnection.GetContext(this.connectionName, provider);
        }
        ...
        /// <returns></returns>
        private string GetScopeId(ICustomErrorPagesEFDataProvider provider)
        {
